...
def cyk_result(sentence: str, grammar: sGCSGrammar, is_sentence_positive: bool, is_learning_on: bool,
               settings: Settings, crowding: Crowding, iteration: Iteration = None):
    manager = multiprocessing.Manager()
    parallelData = ParallelData(manager)
    cyk_values = CykValues(parallelData, grammar, is_learning_on, sentence, settings, crowding, iteration)
    ...
    __parse_sentence(cyk_values, is_sentence_positive, settings)
    result = __get_result(cyk_values)
    cyk_values.is_parsed = __is_parsed(result, is_learning_on, settings)
    cyk_start_cell_rules = cyk_values.rules_table[len(sentence) - 1][0]
...
def init_arrays(cyk_values: CykValues):
    ...
    for i in range(sentence_length):
        ...
        for j in range(sentence_length - i):
            ...
            cyk_values.parallelData.cyk_parsed_cells['{}{}'.format(i, j)] = None
            cyk_values.parallelData.cyk_rules_for_cell['{}{}'.format(i, j)] = []


def __init_rules_table(cyk_values: CykValues):
    cyk_values.rules_table = [[[] for i in range(len(cyk_values.sentence))] for j in range(len(cyk_values.sentence))]
...
def __parse_sentence(cyk_values: CykValues, is_sentence_positive: bool, settings: Settings):
    pool = Pool(cpu_count())
    ...
    for i in range(1, sequence_length):
        job_args = [(i, j,
                     ...
                     cyk_values.parallelData.values) for j in range(sequence_length - i)]
        pool.starmap(__handle_rule, job_args)
        cyk_values.rules_table[i] = [cyk_values.parallelData.cyk_rules_for_cell['{}{}'.format(i, j)] for j in range(sequence_length - i)]
        cyk_values.probability_array[i] = [cyk_values.parallelData.cyk_probability_array['{}{}'.format(i, j)] for j in range(sequence_length - i)]
    pool.close()
    pool.join()


def __handle_rule(i, j, cyk_probability_array, cyk_rules_for_cell, rules_list, values):
    ...
    for rule in rules:
        for k in range(i):
            ...
            if rule.right2 is not None:
                ...
                if fp[first_rule_index] is not None and sp[second_rule_index] is not None:
                    rule.tmp_used = True
                    rule_left_index = rule.left.index

                    cp[rule_left_index] = __calculate_cell(fp[first_rule_index], sp[second_rule_index], cp[rule.left.index], rule)
...
# print('Handling rule {}{} finished in time {}'.format(i, j, end_time - start_time))
...
def __calculate_cell(parent_cell_prob: ProbabilityArrayCell,
    ...
    cell_probability = __calculate_baum_welch_rule_cell_probability(cell_prob,
                                                                    parent_cell_prob,
                                                                    parent_cell_2_prob, rule)
    return cell_probability


def __calculate_baum_welch_rule_cell_probability(cell_probability: ProbabilityArrayCell,
    ...
    while len(cell_rules_to_analyse) > 0:
        ...
        for cell_rule in cell_rules_to_analyse:
            ...
            if cell_rule.cell_1_coordinates is not None:
                ...
                count_1 = len(cyk_values.rules_table[c1x][c1y])
                for i in range(count_1):
                    if cyk_values.rules_table[c1x][c1y][i].rule.left == right1:
                        new_cell_rules.append(cyk_values.rules_table[c1x][c1y][i])
            if cell_rule.cell_2_coordinates is not None:
                ...
                count_2 = len(cyk_values.rules_table[c2x][c2y])
                for i in range(count_2):
                    if cyk_values.rules_table[c2x][c2y][i].rule.left == right2:
                        new_cell_rules.append(cyk_values.rules_table[c2x][c2y][i])
...
def __remove_unused_cell_rules(cyk_values: CykValues):
    ...
    for i in range(sentence_length):
        for j in range(sentence_length):
            for k in reversed(range(len(cyk_values.rules_table[i][j]))):
                if not cyk_values.rules_table[i][j][k].used_in_parsing:
                    cyk_values.rules_table[i][j].pop(k)
...
def __init_rules_value(cyk_values: CykValues, settings: Settings):
    ...
    """
    for i in range(len(cyk_values.sentence)):
        for j in range(len(cyk_values.rules_table[0][i])):
            cyk_values.rules_table[0][i][j].tmp_val = float(settings.get_value('rules', 'base_amount'))
    ...
    for i in range(1, len(cyk_values.sequence)):
        for j in range(len(cyk_values.sequence) - i):
            for k in range(i):
                ...
                for cell_rule_index in range(len(cyk_values.rules_table[i][j])):
                    parent_1 = -1
                    parent_2 = -1
                    # Check each rule in the first possible parent of the cell
                    for parent_rule_index in range(len(cyk_values.rules_table[k][j])):
                        # Check whether the rule right 1 symbol can be created from checked parent rule
                        if cyk_values.rules_table[k][j][parent_rule_index].rule.left.value == \
                                cyk_values.rules_table[i][j][cell_rule_index].rule.right1.value:
                            parent_1 = parent_rule_index

                    # Check each rule in the second posible parent of the cell
                    for parent_rule_index in range(len(cyk_values.rules_table[i - k - 1][j + k + 1])):
                        # Check whether the rule right 2 (if exists) can be be created from checked parent rule
                        if cyk_values.rules_table[i][j][cell_rule_index].rule.right2 is not None and \
                                        cyk_values.rules_table[i - k - 1][j + k + 1][parent_rule_index].rule.left.value \
                                        == cyk_values.rules_table[i][j][cell_rule_index].rule.right2.value:
                            parent_2 = parent_rule_index

                    # If both parents exists
                    if parent_1 != -1 and parent_2 != -1:
                        cyk_values.rules_table[i][j][cell_rule_index].tmp_val = \
                            __compute_new_tmp_value(cyk_values.rules_table[k][j][parent_1],
                                                    cyk_values.rules_table[i - k - 1][j + k + 1][parent_2],
                                                    cyk_values.settings)
...
def __update_rule_profit_and_debt(cyk_values: CykValues, positive: bool):
    ...
    for i in range(len(cyk_values.sequence)):
        for j in range(len(cyk_values.sequence)):
            for k in range(len(cyk_values.rules_table[i][j])):
                if positive is True:
                    cyk_values.rules_table[i][j][k].rule.profit += cyk_values.rules_table[i][j][k].tmp_val
                else:
                    cyk_values.rules_table[i][j][k].rule.debt += cyk_values.rules_table[i][j][k].tmp_val
...
def __calculate_sentence_probability(cyk_values: CykValues, cyk_start_cell_rules, cyk_start_cell_rules_probabilities,
    ...
    start_cell_rules = []
    for i in range(len(cyk_values.rules_table[len(cyk_values.sequence) - 1][0])):
        if cyk_start_cell_rules[i].rule.is_start():
            start_cell_rules.append(cyk_start_cell_rules[i])
...
def __calculate_inside_probabilities(cyk_values: CykValues):
    for i in range(len(cyk_values.sequence)):
        for j in range(len(cyk_values.sequence)):
            for cell_rule in cyk_values.rules_table[i][j]:
                try:
...
def __calculate_outside_probabilities(cyk_values: CykValues, cell_rules_to_analyse):
    ...
    for i in reversed(range(len(cyk_values.sequence))):
        for j in reversed(range(len(cyk_values.sequence))):
            rules_processed = []
            for rule_parent in cyk_values.rules_table[i][j]:
                if rules_processed.__contains__(rule_parent.rule):
                    continue
                rules_processed.append(rule_parent.rule)
                for k in reversed(range(i - 1)):
                    children_1 = [child for child in cyk_values.rules_table[k][j]
                                  if child.rule.left == rule_parent.rule.right1]
                    children_2 = [child for child in cyk_values.rules_table[i - k - 1][j + k + 1]
...
def __find_matching_rules_in_rules_container(cyk_values: CykValues, i_index, j_index, given_symbol):
    ...
    """
    cellRules = []
    for i in range(len(cyk_values.rules_table[i_index][j_index])):
        cellRule: sCellRule = cyk_values.rules_table[i_index][j_index][i]
